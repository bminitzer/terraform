 ...
 * It attaches the Load Balancer created in `alb.tf` to the service, and sets up the networking required.
...
variable "default_backend_image" {
  default = "23525235252.dkr.ecr.us-west-2.amazonaws.com/api:0.1"
}
...
resource "aws_ecs_service" "app" {
  ...
  enable_execute_command            = false # Enable ECS Exec, remove for Prod
...
#Add Rule to RDS Database prod-gpg-aurora Security group
#resource "aws_security_group_rule" "RDS-PROD" {
#  type                     = "ingress"
#  from_port                = 3306
#  to_port                  = 3306
#  protocol                 = "tcp"
#  source_security_group_id = aws_security_group.nsg_task.id
#  security_group_id        = "sg-03ed64513235252"
#}
